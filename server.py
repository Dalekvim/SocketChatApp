...
class InitSocket:
  ...
  def send(self, msg):
    ...
    self.conn.send(SocketConsts().fixed_length_header(message, FORMAT))
  ...
  def handle_client(self, conn, addr):
    ...
    self.send(f"[INFO] Use '{Messages.DISCONNECT_MESSAGE}' to disconnect from the server!")
    ...
    while connected:
      ...
      if msg_length:
        ...
        if connected:
          ...
          self.send(Messages().recieved)
        else:
          self.send(Messages().disconnected_successfully_msg)
